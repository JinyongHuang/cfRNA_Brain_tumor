...
####DESeq2----
df<-data.frame(specimen=c("plasma","plasma","plasma","plasma"),
               case=c("GLI_plasma","MEN_plasma","GLI_plasma","MEN_plasma"),
               control=c("NOR_plasma","NOR_plasma","MEN_plasma","GLI_plasma"),
...
###Distribution----
#"plasma - Gliomas vs. Healthy"
sig_results<-DE_results_distribution[["plasma - Gliomas vs. Healthy"]][["candi_list"]]
...
Type_number_out<-Type_number[Type_number$group== "lncRNA"| Type_number$group== "snoRNA"  ,]
Type_number_in<-Type_number[Type_number$group== "tsRNA" |Type_number$group== "mRNA" | Type_number$group== "snRNA"| Type_number$group== "miRNA" | Type_number$group== "piRNA" ,]
ggplot(Type_number, aes(x="", y=prop, fill=group)) + geom_bar(stat="identity", width=0.5, color="black") + coord_polar("y", start=0) + 
  ...
  labs(title="plasma - Gliomas vs. Healthy") +theme_void() + 
...
ggsave(paste0("Pie plot-plasma - Gliomas vs. Healthy-Candidate results.pdf"), width=6, height = 8,units = "in")
#"plasma - Meningiomas vs. Healthy"
sig_results<-DE_results_distribution[["plasma - Meningiomas vs. Healthy"]][["candi_list"]]
Type_number<-data.frame(group=c("lncRNA","mRNA","miRNA","piRNA","snRNA","snoRNA","tsRNA"),
                        value=c(nrow(sig_results[["lncRNA"]]),nrow(sig_results[["mRNA"]]),nrow(sig_results[["miRNA"]]),nrow(sig_results[["piRNA"]]),
                                nrow(sig_results[["snRNA"]]),nrow(sig_results[["snoRNA"]]),nrow(sig_results[["tsRNA"]])))
Type_number <- Type_number %>% 
...
Type_number$prop2<-paste0(round(Type_number$prop,1),"%")
Type_number$group<-factor(Type_number$group,levels=c("lncRNA","mRNA","miRNA","piRNA","snRNA","snoRNA","tsRNA"))
Type_number_out<-Type_number[Type_number$group== "piRNA" | Type_number$group== "snoRNA" ,]
Type_number_in<-Type_number[Type_number$group== "miRNA" | Type_number$group== "lncRNA" | Type_number$group== "snRNA" | Type_number$group== "tsRNA" |Type_number$group== "mRNA" ,]
ggplot(Type_number, aes(x="", y=prop, fill=group)) + geom_bar(stat="identity", width=0.5, color="black") + coord_polar("y", start=0) + 
  geom_text(data = Type_number_in, aes(y = ypos, label = paste(prop2)), color = "black", size=5, nudge_x = 0.1) +
  geom_label_repel(data = Type_number_out, aes(y = ypos, label = paste0(prop2)), size = 5, nudge_x = 0.6, show.legend = FALSE) +
  scale_fill_manual(values=c("#D6404E","#f78e26","#f7afb9","#F0D43A","#87CFA4","#4E62AB","#8b66b8"))+
  labs(title="plasma - Meningiomas vs. Healthy") +theme_void() + 
  theme(plot.title = element_text(hjust=0.5,face="bold"),legend.text=element_text(size=14), legend.title=element_blank(), legend.position = "bottom")
ggsave(paste0("Pie plot-plasma - Meningiomas vs. Healthy-Candidate results.pdf"), width=6, height = 8,units = "in")