...
names(DE_results)
Gli_Nor<-DE_results[["CSF - Gliomas vs. Healthy"]][["candi_list"]][["cfRNA"]]
Men_Nor<-DE_results[["CSF - Meningiomas vs. Healthy"]][["candi_list"]][["cfRNA"]]
Gli_Men<-DE_results[["CSF - Gliomas vs. Meningiomas"]][["candi_list"]][["cfRNA"]]
Men_Gli<-DE_results[["CSF - Meningiomas vs. Gliomas"]][["candi_list"]][["cfRNA"]]
##gain
...
Venn+labs(title = "CSF - gain")+theme(plot.title = element_text(size=16,face = "bold",hjust=0.5))
ggsave("Venn diagram/csf_Gli_gain.pdf", width=7, height = 7)
ggsave("Venn diagram/csf_Gli_gain.png", width=7, height = 7, bg = "white")
###Gli_loss
Venn_list<-list(
...
Venn <- ggvenn(Venn_list, show_percentage = T, set_name_size = 6, text_size = 5,fill_color = c("red", "orange","yellow"),fill_alpha = 0.5)
Venn+labs(title = "CSF - loss")+theme(plot.title = element_text(size=16,face = "bold",hjust=0.5))
ggsave("Venn diagram/csf_Gli_loss.pdf", width=7, height = 7)
ggsave("Venn diagram/csf_Gli_loss.png", width=7, height = 7, bg = "white")
...
Venn <- ggvenn(Venn_list, show_percentage = T, set_name_size = 6, text_size = 5,fill_color = c("red", "orange","yellow"),fill_alpha = 0.5)
Venn+labs(title = "CSF - gain")+theme(plot.title = element_text(size=16,face = "bold",hjust=0.5))
ggsave("Venn diagram/csf_Men_gain.pdf", width=7, height = 7)
ggsave("Venn diagram/csf_Men_gain.png", width=7, height = 7, bg = "white")
###Men_loss
Venn_list<-list(
  `Gli vs. Nor`=Gli_Nor_loss,
  `Men vs. Nor`=Men_Nor_loss,
...
Venn <- ggvenn(Venn_list, show_percentage = T, set_name_size = 6, text_size = 5,fill_color = c("red", "orange","yellow"),fill_alpha = 0.5)
Venn+labs(title = "CSF - loss")+theme(plot.title = element_text(size=16,face = "bold",hjust=0.5))
ggsave("Venn diagram/csf_Men_loss.pdf", width=7, height = 7)
ggsave("Venn diagram/csf_Men_loss.png", width=7, height = 7, bg = "white")
    ...
    result_seed<-foreach(i=N, .combine="c",.packages=c("tidyverse","ggplot2","ggpubr","rstatix","reshape2")) %dopar% {
      ...
      if (table(variance$x>5)['FALSE']==6) {
        ...
        if (table(sum$x==0)['FALSE']==6) {
          ...
          if (name=="BrainTumor" & stat.test$p[2]<0.05 & stat.test$p[3]<0.05) {
          ...
          if (name=="Gliomas" & stat.test$p[1]<0.05 & stat.test$p[2]<0.05) {
            plot <- ggplot(df, aes(x=Group, y=RNA)) + geom_boxplot(aes(fill = Group),outlier.shape=NA)+
              scale_fill_manual(values=c("#E31A1C","#FF7F00","#1F78B4","#FB9A99","#FDBF6F","#A6CEE3"))+
              geom_jitter(shape=16, position=position_jitter(0.2))+
              labs(title=paste0(colnames(subset)[i]), x="", y = expression('Log'[2]*' (RPM)'))+ theme_classic()+
          ...
          if (name=="Meningiomas" & stat.test$p[1]<0.05 & stat.test$p[3]<0.05) {
            plot <- ggplot(df, aes(x=Group, y=RNA)) + geom_boxplot(aes(fill = Group),outlier.shape=NA)+
              scale_fill_manual(values=c("#E31A1C","#FF7F00","#1F78B4","#FB9A99","#FDBF6F","#A6CEE3"))+
              geom_jitter(shape=16, position=position_jitter(0.2))+
              labs(title=paste0(colnames(subset)[i]), x="", y = expression('Log'[2]*' (RPM)'))+ theme_classic()+
...
saveRDS(ploted_goi,"ploted_goi_csf_Gliomas.rds")
writeLines(ploted_goi,"ploted_goi_csf_Gliomas.txt")
ploted_goi<-list.files("Boxplot/Meningiomas/",pattern = ".png",recursive = TRUE)
ploted_goi<-word(ploted_goi,start = 2,sep = "/")
ploted_goi<-gsub(".png","",ploted_goi)
saveRDS(ploted_goi,"ploted_goi_csf_Meningiomas.rds")
writeLines(ploted_goi,"ploted_goi_csf_Meningiomas.txt")
ploted_goi<-list.files("Boxplot/BrainTumor/",pattern = ".png",recursive = TRUE)
ploted_goi<-word(ploted_goi,start = 2,sep = "/")
ploted_goi<-gsub(".png","",ploted_goi)
saveRDS(ploted_goi,"ploted_goi_csf_BrainTumor.rds")
writeLines(ploted_goi,"ploted_goi_csf_BrainTumor.txt")
