...
meta <- meta[grep("CSF",meta$Group,invert = TRUE),]##
table(meta$Group)
meta$group<-ifelse(meta$Group=="NOR_plasma","Healthy","Braintumor")##
###Discovery cohort
dis_case<-subset(meta,Group=="NOR_plasma")##
dis_control<-subset(meta,Group!="NOR_plasma")##
dis_ID<-c(dis_case$seqID,dis_control$seqID)
dis_type<-factor(c(dis_case$group,dis_control$group),levels = c("Braintumor","Healthy"))##
ndiscovery<-length(dis_ID)

####cfRNA of interest 
goi_cfRNA<-readRDS("../../DESeq2_One_vs_One_plasma/plasma_intersection/ploted_goi_plasma_BrainTumor.rds")
...
for (N in as.character(condition)) {
  ...
  ggplot(df, aes(x=variable, y= value,colour=variable))+geom_boxplot(outlier.shape = NA)+  geom_jitter(shape=16, position=position_jitter(0.2)) + facet_wrap(~method, scale="free")+
    scale_colour_brewer(palette="Set1")+labs(title=paste0("plasma - Brain tumor vs. Healthy - ",n, " cfRNA") ,x="", y = "AUC in 100 iterations")+ theme_classic()+
...
saveRDS(LR_roc,"plasma_Braintumor_vs_Healthy_p=0.6_LASSO1_training_ROC.rds")
saveRDS(LR_AUC,"plasma_Braintumor_vs_Healthy_p=0.6_LASSO1_training_AUC.rds")
#testing
LR_prob <- predict(Ridge, testing_x, type = "response")[,1]
LR_roc <- plot.roc(testing_y,LR_prob)
LR_pred <- prediction(LR_prob, testing_y)
LR_AUC <- round(attr(performance(LR_pred, "auc"), "y.values")[[1]],3)
saveRDS(LR_roc,"plasma_Braintumor_vs_Healthy_p=0.6_LASSO1_testing_ROC.rds")
saveRDS(LR_AUC,"plasma_Braintumor_vs_Healthy_p=0.6_LASSO1_testing_AUC.rds")

##
summary<-as.data.frame(All[["1"]])
