...
table(meta$Group)
meta$group<-ifelse(meta$Group=="MEN_CSF","Meningiomas","others")##
###Discovery cohort
dis_case<-subset(meta,Group=="MEN_CSF")##
dis_control<-subset(meta,Group!="MEN_CSF")##
dis_ID<-c(dis_case$seqID,dis_control$seqID)
dis_type<-factor(c(dis_case$group,dis_control$group),levels = c("Meningiomas","others"))##
ndiscovery<-length(dis_ID)

####cfRNA of interest 
goi_cfRNA<-readRDS("../../DESeq2_One_vs_One_csf/CSF_intersection/ploted_goi_csf_Meningiomas.rds")
...
for (N in as.character(condition)) {
  ...
  ggplot(df, aes(x=variable, y= value,colour=variable))+geom_boxplot(outlier.shape = NA)+  geom_jitter(shape=16, position=position_jitter(0.2)) + facet_wrap(~method, scale="free")+
    scale_colour_brewer(palette="Set1")+labs(title=paste0("CSF - Meningiomas vs. Others - ",n, " cfRNA") ,x="", y = "AUC in 100 iterations")+ theme_classic()+
...
saveRDS(LR_roc,"CSF_Meningiomas_vs_others_p=0.6_LASSO5_training_ROC.rds")
saveRDS(LR_AUC,"CSF_Meningiomas_vs_others_p=0.6_LASSO5_training_AUC.rds")
#testing
LR_prob <- predict(Ridge, testing_x, type = "response")[,1]
LR_roc <- plot.roc(testing_y,LR_prob)
LR_pred <- prediction(LR_prob, testing_y)
LR_AUC <- round(attr(performance(LR_pred, "auc"), "y.values")[[1]],3)
saveRDS(LR_roc,"CSF_Meningiomas_vs_others_p=0.6_LASSO5_testing_ROC.rds")
saveRDS(LR_AUC,"CSF_Meningiomas_vs_others_p=0.6_LASSO5_testing_AUC.rds")
#all
LR_prob <- predict(Ridge, discovery_x, type = "response")[,1]
predicted_classes <- ifelse(LR_prob > 0.5, "others", "Meningiomas")
conf_matrix <- table(Predicted = predicted_classes, Actual = discovery_y)
conf_matrix
