...
csf_goi<-readLines("../CSF_Braintumor_vs_Healthy_p=0.6/goi/5-name.txt")##
ncsf<-length(csf_goi)
...
plasma_goi<-readLines("../plasma_Braintumor_vs_Healthy_p=0.6/goi/1-name.txt")##
...
###Discovery cohort
meta$group<-ifelse(meta$Group=="NOR","Healthy","Braintumor")##
dis_case<-subset(meta,Group!="NOR")##
dis_control<-subset(meta,Group=="NOR")##
...
saveRDS(LR_roc,"CSF&plasma_Braintumor_vs_Healthy_p=0.6_training_ROC.rds")##
saveRDS(LR_AUC,"CSF&plasma_Braintumor_vs_Healthy_p=0.6_training_AUC.rds")##
#testing
LR_prob <- predict(Ridge, testing_x, type = "response")[,1]
LR_roc <- plot.roc(testing_y,LR_prob)
LR_pred <- prediction(LR_prob, testing_y)
LR_AUC <- round(attr(performance(LR_pred, "auc"), "y.values")[[1]],3)
saveRDS(LR_roc,"CSF&plasma_Braintumor_vs_Healthy_p=0.6_testing_ROC.rds")##
saveRDS(LR_AUC,"CSF&plasma_Braintumor_vs_Healthy_p=0.6_testing_AUC.rds")##
